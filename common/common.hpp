...
enum class MAV_CMD : uint16_t
{
    ...
    NAV_FLYINGROVER_TAKEOFF=86, /* Takeoff from ground using flying rover mode, and transition to multicopter flight with specified heading. |Empty| Front transition heading.| Empty| Yaw angle. NaN to use the current system yaw heading mode (e.g. yaw towards next waypoint, yaw to home, etc.).| Latitude| Longitude| Altitude|  */
    NAV_FLYINGROVER_LAND=87, /* Land using flyingrover mode |Empty| Empty| Approach altitude (with the same reference as the Altitude field). NaN if unspecified.| Yaw angle. NaN to use the current system yaw heading mode (e.g. yaw towards next waypoint, yaw to home, etc.).| Latitude| Longitude| Altitude (ground level)|  */
    ...
    DO_FLYINGROVER_TRANSITION=3100, /* Request flying rover transition |The target flying rover state. Only MAV_FLYINGROVER_STATE_MC and MAV_FLYINGROVER_STATE_ROVER can be used.| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)|  */
    SET_ROVER_FORWARD_REVERSE_DRIVING=3101, /* Set forward or reverse driving status for rover mode  |The forward/reverse driving state in rover mode, set 1 for forward, 0 for reverse| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)|  */
...
enum class MAV_FLYINGROVER_STATE : uint8_t
{
    UNDEFINED=0, /* MAV is not configured as flyingrover | */
    ROVER=1, /* flyingrover is rover state | */
    MC=2, /* flyingrover is in multicopter state | */
    TRANSITION_TO_MC=3, /* flyingrover is in transition from rover to multicopter | */
    TRANSITION_TO_ROVER=4, /* flyingrover is in transition from multicopter to rover | */
};
...
/** @brief Direction of VTOL transition */
enum class FLYINGROVER_TRANSITION_HEADING
{
    VEHICLE_DEFAULT=0, /* Respect the heading configuration of the vehicle. | */
    NEXT_WAYPOINT=1, /* Use the heading pointing towards the next waypoint. | */
    TAKEOFF=2, /* Use the heading on takeoff (while sitting on the ground). | */
...
//! FLYINGROVER_TRANSITION_HEADING ENUM_END
constexpr auto FLYINGROVER_TRANSITION_HEADING_ENUM_END = 5;
...
#include "./mavlink_msg_avoidance_status.hpp"
#include "./mavlink_msg_vcu_base_status.hpp"
#include "./mavlink_msg_vcu_command_velocity.hpp"
#include "./mavlink_msg_vcu_bms_status.hpp"
#include "./mavlink_msg_vcu_direct_can_ctrl.hpp"
